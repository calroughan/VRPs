...
def my_branch(prob, sol):
    ...
    # Priority y branches work far better for cart 3 problems
    if prob.vrp.allused and "Priority" in options:
        options["Priority"] = "y"
    ...
    if (bounds is None) and ("Priority" in options) and (options["Priority"] == "x"):
        bounds = x_branch(prob, sol)

    if (bounds is None) and ("Priority" in options) and (options["Priority"] == "y"):
        bounds = y_branch(prob, sol, True)

    return bounds


def x_branch(prob, sol):
